...
#region Fish Manager
    public class FishMan{
        private List<UCell>   pBDL;             //SuDoKu cells   (for reference)    
        private Bit81[]       pHouseCells;      //houseCells     (for reference) 
        private Bit81[]       pConnectedCells;  //connectedCells (for reference) 

        private int           sz;               //fish size (2D:Xwing 3D:SwordFish 4D:JellyFish 5D:Squirmbag 6D:Whale 7D:Leviathan)
        private int           no;               //digit

        private List<Bit81F>  HBLst=new List<Bit81F>(); //element cells of fish(bit expression)
        private bool extFlag;                   //3D or more sizes. ...

        public FishMan( AnalyzerBaseV2 AnB, int FMSize, int no, int sz, bool extFlag=false ){
            this.pBDL = AnB.pBDL;
            this.pHouseCells = AnalyzerBaseV2.HouseCells;           //set reference
            this.pConnectedCells = AnalyzerBaseV2.ConnectedCells;   //set reference
            this.extFlag=extFlag;                                   //sz>2 or ...                 
            this.no=no; this.sz=sz;
            int noB=(1<<no);                                        //Bit expression of dijit(#no)

            Bit81 Q, BPnoB=new Bit81(pBDL,noB);
            for(int tfx=0; tfx<FMSize; tfx++ ){                     //crate element cells of fish
                Q = pHouseCells[tfx]&BPnoB;
                if( !Q.IsZero() && !HBLst.Contains(Q) ) HBLst.Add(new Bit81F(Q,ID:tfx));//Omit the same cells even in different House
            }
...
#region Fish
    public class UFish{
        ...
        public Bit81    EndoFin=null;   //Bit expression of EndoFin cells
        ...
        public Bit81    CannFin=null;   //Bit expression of cannibalisticFin cells
