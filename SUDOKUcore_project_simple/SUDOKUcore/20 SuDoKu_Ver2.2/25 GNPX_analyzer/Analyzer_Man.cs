...
//==========================================================
        public bool SnapSaveGP( bool saveAll=false ){
            ...
            try{
                if( !SDK_Ctrl.MltAnsSearch ){ // ***** not SDK_Ctrl.MltAnsSearch *****
                    UPuzzle GPX = pGP.Copy(pGP.stageNo+1,0);                // GPX:Next analysis target phase
                    SDK_Ctrl.UGPMan.pGPsel = GPX;                           // Set the next phase(GPX) in the engine
                    GPX.__SolRes = GPX.Sol_ResultLong.Replace("\r"," ");    // Adjustment of solution message               
                    return false;
                }
                ...
                if( SDK_Ctrl.UGPMan.MltUProbLst.Count>=(int)SDK_Ctrl.MltAnsOption["AllMethod"] ){
                    SDK_Ctrl.MltAnsOption["abortResult"] = pRes.msgUpperLimitBreak; // Reached the limit of multiple solution searches.
                    return false;
                }    
